...
impl Client {
                        ...
                        if event.token() == request.token {
                            ...
                            if event.is_readable() {
                                ...
                                if let Some((head, body_begin)) = ResponseHead::parse(&request.current_result) {
                                    ...
                                    // maybe some or all of the body was also received

                                    let mut is_done = false;

                                    if request.body_bytes_read > 0 {
                                        let (data, is_done_inner) = Self::process_request(request);
                                        is_done = is_done_inner;
                                        responses.push(Response::new(request.id, ResponseState::Data(data)));
                                    }

                                    if request.body_length == 0 && !request.transfer_chunked {
                                        is_done = true;
                                    }

                                    if is_done | was_closed {
                    ...
                    InternalRequestState::RecvBody => {
                        ...
                        if event.token() == request.token {
                            ...
                            if event.is_readable() {
                                ...
                                let (data, is_done) = Self::process_request(request);
    ...
    fn process_request<'d>(request: &'d mut InternalRequest) -> (/* data */ Vec<u8>, /* is_done */ bool) {
        ...
        if request.transfer_chunked {
            ...
            request.body_bytes_read += request.current_result.len(); // does he suck?
